...
def log_posterior_probs(x):
    ...
    # p(H|x) = 1/Z * p(x|H) * p(H)
    log_p = [logsumexp(HYPOTHESIS_SPACE[0].pdf(x))+np.log(PRIOR_PROBS[0]),
             logsumexp(HYPOTHESIS_SPACE[1].pdf(x))+np.log(PRIOR_PROBS[1]),logsumexp(HYPOTHESIS_SPACE[2].pdf(x))+np.log(PRIOR_PROBS[2])]
    log_p = np.array(log_p)
    total = sum(np.exp(log_p))
    log_p = log_p-np.log(total)
