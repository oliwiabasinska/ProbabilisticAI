...
def log_posterior_probs(x):
    ...
    # p(H|x) = 1/Z * p(x|H) * p(H)
    log_p = [sum(HYPOTHESIS_SPACE[0].logpdf(x))+np.log(PRIOR_PROBS[0]),
             sum(HYPOTHESIS_SPACE[1].logpdf(x))+np.log(PRIOR_PROBS[1]),sum(HYPOTHESIS_SPACE[2].logpdf(x))+np.log(PRIOR_PROBS[2])]
